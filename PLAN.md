...
## ▶ Текущий шаг
Сделано: тулчейн ✅, **0.2** ✅, **1.1–1.4** ✅ (**12 LiteSVM-тестов зелёные**). 1.4 ввёл `anchor-spl`,
USDC-vault (PDA-токен-аккаунт), аккаунты `Proposal`/`Bid`, перевод `transfer_checked` в vault; mint/ATA в тестах
через `litesvm-token`. **Vault создаётся в `create_auction`** (не lazy), чтобы не тащить `init_if_needed` (минимальнее аудит).
Дальше — **1.5** `place_bid` на **существующее** предложение (добор/перебой) — это отдельная инструкция
(`raise_bid`/`back_proposal`): новый биддер на чужое предложение + повторная ставка тем же биддером (накопление в Bid).
...
- [ ] 1.5 `place_bid` добор/перебой (**отдельная инструкция** на существующее предложение). Тест: суммы накапливаются; лидер меняется; повторная ставка тем же биддером (Bid накапливается, `init_if_needed` или отдельный путь).
