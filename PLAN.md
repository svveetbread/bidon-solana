...
## ▶ Текущий шаг
**✅ Phase 1 (ядро Anchor-программы) ПОЛНОСТЬЮ ГОТОВА** — **8 инструкций, 37 LiteSVM-тестов зелёные, без warnings.**
...
**Газлесс Этап 2a ✅** — `withdraw` ЗАКРЫВАЕТ Bid (лузеру ставка USDC, relayer'у rent назад по `rent_payer`; `returned` убран —
закрытие само блокирует повтор). Тест `relayer_recovers_rent_on_withdraw`.

**Дальше:**
- **Газлесс Этап 2b** — `close_auction` (permissionless, после расчёта): закрыть vault + Auction + победившее Proposal/Bid,
  rent → relayer. Нужен счётчик незакрытых Bid (знать, что всё расселено). Сложнее — GC аккаунтов.
- **Газлесс Этап 3 (Phase 2)** — поднять **Kora** (self-host relayer), обернуть TS-клиент (ко-подпись payer).
