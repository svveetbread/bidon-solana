...
## ▶ Текущий шаг
**✅ Phase 1 (ядро Anchor-программы) ПОЛНОСТЬЮ ГОТОВА** — **8 инструкций, 36 LiteSVM-тестов зелёные, без warnings.**
...
Тест `test_relayer.rs` (2): юзер тратит 0 SOL, relayer платит. **Ставку 0.1 USDC делать можно — юзер SOL не касается.**

**Дальше:**
- **Газлесс Этап 2** — закрытие аккаунтов с возвратом rent **relayer'у** (`rent_payer`): `withdraw` закрывает Bid;
  `close_auction` (permissionless, после расчёта) — vault/Auction/Proposal. Нужен счётчик незакрытых Bid.
