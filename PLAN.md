...
## ▶ Текущий шаг
Сделано: тулчейн ✅, **0.2** ✅, **1.1–1.9** ✅ (**32 LiteSVM-теста зелёные**). **Ядро функционально полное —
8 инструкций.** 1.4 `place_bid` · 1.5 `raise_bid` + лидер · 1.6 негативы · 1.7 `finalize` · 1.8 `claim_winnings`
(creator − fee) · 1.9 `withdraw` (loser забирает ставку; победившие — нет; PDA-вывод; гейты `finalized`+`!returned`; permissionless).
Тест-харнес — в **`tests/common/mod.rs`**. Все outbound из vault — подпись авторитетом-аукционом (`new_with_signer`).
Дальше — **1.10** сводный e2e-инвариант: полный цикл (создание→ставки→финал→claim→все withdraw),
проверка `Σ in == Σ out` и `vault → 0`. Затем **1.11** — общий `cargo test` зелёный (уже по факту).
