...
## ▶ Текущий шаг
**✅ Phase 1 (ядро Anchor-программы) ПОЛНОСТЬЮ ГОТОВА** — **8 инструкций ядра (+3 GC-закрытия = 11), 38 LiteSVM-тестов зелёные, без warnings.**
...
**✅ Сделано после Phase 1:** security-проход (выше) · 5.1 e2e на реальном рантайме (см. ниже) ·
**газлесс Этап 1** — в `create_auction`/`place_bid`/`raise_bid` отдельный `payer: Signer` (relayer платит rent+fee),
юзер (creator/bidder) с **0 SOL** только подписывает как authority; `rent_payer` хранится в аккаунте (возврат — relayer'у).
Тест `test_relayer.rs`: юзер тратит 0 SOL, relayer платит. **Ставку 0.1 USDC делать можно — юзер SOL не касается.**
**Газлесс Этап 2a ✅** — `withdraw` ЗАКРЫВАЕТ Bid (лузеру ставка USDC, relayer'у rent назад; `returned` убран).
**Газлесс Этап 2b ✅** — permissionless GC: `close_bid` (победившие биды), `close_proposal`, `close_auction` (vault+Auction через
`spl close_account` PDA-подписью). Каждый rent → своему `rent_payer` (relayer). Кипер крэнкает по порядку (биды→предложения→аукцион);
без счётчика — мис-крэнк осиротит rent (потеря relayer'а, НЕ юзера; не fund-риск). **Полный возврат rent доказан** (`relayer_recovers_all_rent_via_close`).
**🪙 Газлесс замкнут: relayer платит rent → возвращает при закрытии (цикл), юзер 0 SOL. Программа — 11 инструкций.**

**Экономика relayer'а (решено):** юзер платит ставку + копеечный газ в **USDC** (~$0.0007, невозвратный) — это **конфиг Kora**, БЕЗ правок программы;
rent гоняет свой **Kora** (фронт+возврат, цикл ≈ 0). Покрывается feeBps платформы. Relayer = свой бэкенд (Kora self-host), кошелёк с оборотным SOL.

**✅ Газлесс e2e на реальном рантайме** (`e2e/e2e.mjs`, передеплой 11 инструкций): relayer=провайдер платит всё,
creator+биддеры с **0 SOL** подписывают; полный цикл + закрытия; проверено `getBalance(user)===0`, relayer вернул rent. Все зелёные.

**Дальше:**
- **Газлесс Этап 3 (Phase 2)** — поднять **Kora** (self-host relayer), газ-в-USDC, обернуть TS-клиент (ко-подпись payer).
- **Публичный devnet** — деплой+e2e (ждёт SOL: приватный RPC/кулдаун; фасет блокирует агентов).
- **Phase 2 фронт** (реюз `D:\bidon-base\frontend`, Anchor TS-клиент — посеян в `e2e/`).
- Обновить `e2e/e2e.mjs` под новый `payer` (+ передеплой) — газлесс e2e на реальном рантайме.
...
**Линковка:** правка `common/mod.rs` релинкует все тест-бинарники разом → параллельный `ld` жрёт >7.7ГБ RAM → OOM
(`final link failed`, НЕ диск/код). Фикс: `cargo test --jobs 1` в `build.sh` (серийная линковка). Одиночный линк проходит.
...
- [~] 5.1 Полный e2e на **реальном рантайме** ✅ (локальный валидатор — те же SPL-программы/деплой/PDA-подпись, что devnet). Программа задеплоена (upgradeable), TS-клиент `@coral-xyz/anchor` (`e2e/e2e.mjs`), 13/13 проверок: создание→ставки+добор→лидер→финал по времени→claim (14.445+0.555)→withdraw→`vault→0`. **Публичный devnet — ждёт SOL** (фасет блокирует агентов; нужен приватный RPC/кулдаун).
