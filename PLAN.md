...
## ▶ Текущий шаг
Сделано: тулчейн ✅, **0.2** ✅, **1.1–1.3** ✅ (9 LiteSVM-тестов зелёные; create_auction + счётчик в Config).
Дальше — **1.4** `place_bid` (тут вводим `anchor-spl` + USDC-vault + mint в тестах через `litesvm-utils`).
