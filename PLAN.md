...
## ▶ Текущий шаг
Сделано: тулчейн ✅, **0.2** ✅, **1.1–1.8** ✅ (**28 LiteSVM-тестов зелёные**). 1.4 — `anchor-spl`,
USDC-vault, `Proposal`/`Bid`, `transfer_checked`. 1.5 — `raise_bid` + инкрементальный лидер. 1.6 — негативы.
1.7 — `finalize`. 1.8 — `claim_winnings` (creator получает `winner_amount − fee`, комиссия → fee_receiver;
**outbound из vault через PDA-подпись авторитетом-аукционом**, `CpiContext::new_with_signer`; permissionless; только раз).
Тест-харнес — в **`tests/common/mod.rs`**.
Дальше — **1.9 `withdraw`** (проигравший забирает ставку: тот же PDA-вывод; гейты `finalized` + `bid.proposal != winner` +
`!returned`; permissionless, биддер задаётся аргументами) → **1.10** сводные инварианты (`Σin==Σout`, `vault==Σ невозвращённых`).
