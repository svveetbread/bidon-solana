...
## ▶ Текущий шаг
Phase 0: тулчейн (rust 1.96 ✅, solana-cli 4.0.1 ✅, apt-deps ✅; Anchor ставится) → дальше **0.2**.
...
- [ ] 0.2 `anchor init bidon` → `anchor build` собирается + дефолтный тест зелёный на локальном валидаторе. (Assumption #3: мы вообще собираем под Solana.)
...
- [ ] 1.1 `initialize` Config (owner/fee_bps/fee_receiver/usdc_mint). Тест: поля; только owner; повторно нельзя.
- [ ] 1.2 `set_config` (owner): тест границ fee (max 1000 bps = 10%); только owner.
