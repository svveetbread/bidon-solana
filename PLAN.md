...
## ▶ Текущий шаг
Сделано: тулчейн ✅, **0.2** ✅, **1.1–1.7** ✅ (**24 LiteSVM-тестов зелёные**). 1.4 — `anchor-spl`,
USDC-vault, `Proposal`/`Bid`, `transfer_checked`. 1.5 — `raise_bid` (`init_if_needed` Bid) + инкрементальный лидер.
1.6 — негативные валидации. 1.7 — `finalize` (permissionless после end_time; победитель = `winner_proposal`; идемпотентно).
Тест-харнес — в **`tests/common/mod.rs`**.
Дальше — **1.8 `claim_winnings`** (creator забирает победный пул − fee; **outbound из vault → PDA-подпись авторитетом
аукциона**, новый паттерн `new_with_signer`) → **1.9 `withdraw`** (loser забирает ставку) → 1.10 инварианты. Раздача — pull + keeper.
